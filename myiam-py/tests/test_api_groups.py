...
def test_update_group_attach_policies(ddbt):
    create_group(ddbt, group_name="sales", human_name="Sales")
    update_group_attach_policies(ddbt, group_name="sales", policy_names=["P1", "P2"])
    group = describe_group(ddbt, group_name="sales")
    print(group)


def test_update_group_detach_policies(ddbt):
    create_group(ddbt, group_name="sales", human_name="Sales")
    update_group_attach_policies(ddbt, group_name="sales", policy_names=["P1", "P2"])
    update_group_detach_policies(ddbt, group_name="sales", policy_names=["P1", "P2"])
    group = describe_group(ddbt, group_name="sales")
    print(group)
